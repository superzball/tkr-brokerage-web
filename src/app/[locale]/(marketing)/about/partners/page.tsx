...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({
    locale,
    namespace: "metadata.aboutPartners",
  });
    ...
    <main>
      ...
      <section className="bg-hero border-b border-ink-100/70">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-14 lg:py-20 text-center">
          ...
          <h1
            className="font-display font-700 text-3xl sm:text-5xl text-ink-900 tracking-tight"
      ...
      {/* partner logo wall — real rights-cleared insurer logos, grouped */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-14 lg:py-20">
        ...
        {partners.length === 0 ? (
          ...
          <>
            {/* full grouped list of all partners — each shows its real logo tile */}
            <div className="space-y-6">
              {([1, 2, 3] as const).map((g) => {
                ...
                return (
                  <div
                    key={g}
                    className={g > 1 ? "pt-6 border-t border-ink-100" : ""}
                  >
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {list.map((p) => (
                        ...
                        >
                          {/* real rights-cleared logo — wide white tile so horizontal
                              marks (MSIG/Aioi/Tokio Marine) read at a glance */}
                          <span className="shrink-0  rounded-xl bg-white border border-ink-100 flex items-center justify-center p-0.5">
                            {p.logo ? (
                              <Image
                                src={p.logo}
                                alt={p.name}
                          ...
                          <span className="font-600 text-sm text-ink-800 leading-tight">
                            {p.name}
                          </span>
                        </div>
                      ))}
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 py-14">
        <div className="card p-7 sm:p-9 flex items-start gap-4">
          ...
          <div>
            <h3 className="font-display font-700 text-xl text-ink-900">
              {t("underwriter.title")}
            </h3>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 pb-16">
        <div className="card card-lg bg-gradient-to-br from-brand-600 to-ink-900 text-white p-9 sm:p-12 text-center">
          <h2
            className="font-display font-700 text-2xl sm:text-3xl tracking-tight"
            style={{ textWrap: "balance" }}
          >
            {t("cta.title")}
          </h2>
